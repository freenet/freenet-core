...
2. **~~Measure actual metadata overhead~~** ✅ **Implemented in PR #2271**
   ...
   - **Benchmark results** (local Criterion, 100 iterations):
     - 256-byte messages: **+6.27% throughput** (biggest gain)
     - 1364-byte messages: **+2.21% throughput**
     - Network efficiency: fewer fragments needed for large messages
   - See `symmetric_message::stream_fragment_overhead()` for measurement implementation
