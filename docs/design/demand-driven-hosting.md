...
**Status: canonical design-intent doc for the demand-driven hosting/subscription model. Converged with Ian on 2026-07-01 and carrying the eviction, reads-as-demand, and SUBSCRIBE-retirement decisions settled through 2026-07-08. This is the WHY-and-HOW reference the invariants file points at; the code does not yet fully implement it (that is the redesign, tracked in the epic below). Where this doc and `.claude/rules/hosting-invariants.md` differ, the invariants file is newer and wins.**
...
- Detailed build spec (mechanisms, constants, `file:line` cites): `docs/design/hosting-spec.md`
...
A peer hosts a contract because **real demand routed to it**. "Real demand" is one of:

- a **local client** on that peer reading or subscribing to the contract,
- a **downstream subscriber** (another peer, farther from the contract's key, that has active demand of its own) routing a subscription through it, or
- a **routed GET or PUT** that passed through the peer: a real read or write is itself demand, so every hop on a GET/PUT route starts hosting the contract (every-hop placement).

The first two drive the **subscription chain** of sections 3–6; the third places an **every-hop copy** retained under the demand gauge of section 9. What a peer does **not** do is keep a copy **durable regardless of demand**: it does not retain a contract merely because it once forwarded a request, because a neighbor pushed it, or because it relayed traffic for it. The every-hop copy from a routed GET/PUT is held under that gauge (retained by its subscriber count and its GET/PUT recency, and the first thing evicted once it has neither), which is exactly what separates legitimate every-hop hosting from the durable relay-caching anti-pattern (section 9). Replication degree is not a tunable global constant; it emerges from where demand exists and how requests route.
...
A peer sends renewals toward its upstream **only while it is itself `contract_in_use`**, where `contract_in_use` means: it has a local client interested in the contract, **or** it has a registered downstream subscriber (a peer strictly farther from the key than itself), **or** it has recent **real GET/PUT access** (a genuine client read or write that routed through it; automatic renewal traffic does not count). This is the *same* predicate that decides whether the peer keeps hosting (section 6): one condition drives both. A read-only or PUT-only contract that is never subscribed (the River UI container and the web/UI container contracts) is therefore kept renewed by its GET/PUT recency alone, and does not fall out of the mesh for lack of a subscription, consistent with invariant 3.

When a peer's demand dries up (its client leaves, its last downstream lapses, and its GET/PUT recency ages out), it stops renewing. Its registration at the upstream then lapses on its own (downstream registrations lease-expire, see section 6). That removes one unit of demand from the upstream, which may in turn cause the upstream to fall out of `contract_in_use`, stop renewing, and lapse at *its* upstream. The effect **propagates inward toward the key**, collapsing exactly the part of the chain that no longer has demand behind it.
...
**Degradation, no deadlock.** There is no deadlock and no separate admission gate to refuse the subscribe. A peer closer to the key that the subscribe reaches simply takes it and lets it compete in the single eviction (piece A, section 9), evicting its own lowest-ranked contract if it is over budget (a fewest-subscriber arrival that nothing outranks is the one dropped, an emergent refusal rather than an explicit NACK). Where routing cannot reach an ideal closer host (the ~5-9% near-miss floor), the peer simply remains a **valid stranded host serving its own mesh component** until a closer peer can take it, with terminal-advertisement consult (piece C, section 9) keeping the contract findable in the meantime. The worst case is **suboptimal placement** (the contract is hosted farther from its key than ideal), never unavailability.

---

## 6. The stop-hosting / collapse rule

A peer **keeps hosting** a contract **iff** it has any of:

- a **local client** interested in the contract,
- a registered subscriber **strictly farther** from the contract's key than itself (a downstream), **or**
- recent **real GET/PUT access** (a genuine client read or write that routed through it; automatic renewal traffic does not count).

It **stops** hosting when it has none of these. Critically, the **upstream (any neighbor closer to the key) is explicitly excluded** from what can keep it hosting. Note this is the same `contract_in_use` predicate as the renewal gate in section 5a: a peer keeps hosting exactly as long as it keeps renewing, and stops both together.
...
- **Piece A, eviction/retention and the capability budget.** **Base shipped in 0.2.90** (telemetry #4643, RAM-sized budget #4644, demand-ordered eviction #4650). Documented in `docs/design/hosting-eviction.md`. A bounds **memory/slots**; it deliberately does **not** touch the renewal storm (section 7). A had to ship and be proven in the field first, because the pieces below assume its bound is already live (every-hop hosting without A's gauge would be relay-caching; host-chains without A's budget would be the storm).
  - **Eviction is ONE demand-ordered decision.** It orders victims ascending `(local_subscription_count, downstream_subscriber_count, recency, key bytes)`: fewest **local client subscriptions** first (a contract the peer's own client is subscribed to is evicted LAST), then fewest **downstream subscribers**, then least-recent **real GET or PUT access** (recency), then contract-key bytes as a final deterministic tiebreak. Local and downstream subscribers are **separate ranking dimensions** (local above downstream) because a local subscription cannot re-home; neither is an absolute pin.
  - **Reads and PUTs are a permanent demand signal, not just a recency tiebreak.** A contract that is only ever GET-read or PUT and never subscribed (the **River UI container** and the **web/UI container** contracts are the canonical case) is kept renewed in the update mesh and retained by its GET/PUT recency, and must **not** fall out of the network for lack of a subscription. Subscriptions are the higher-priority signal: while a contract has an active subscription its recency clock does **not** tick (it is held by the subscription tier); the clock starts only at subscription **termination**, and thereafter (and for never-subscribed read-only contracts) retention is by real GET/PUT recency. Only genuine client access (a real GET or PUT) resets recency; automatic subscription-renewal traffic must not. There is **no `min_ttl` cold-start floor**: a real GET or PUT resets recency, so a freshly-accessed contract is protected simply by being the most-recently-accessed, and a fresh PUT resets recency at PUT time without waiting for a first read. This **replaces** the earlier framing that a "heavily-read but never-subscribed" contract was transient cruft superseded by "subscribe ⇒ pinned": read-only demand (the UI containers) is now first-class and retained, and re-reading is genuine demand.
  - **Distance is removed from the eviction ranking**: its causal effect on demand already flows through subscriber count (subscriptions route toward the key, so near-key peers accumulate more subscribers, and counting both would double-count), and network locality is delivered by **routing** (sections 2–4), not by eviction. The earlier "predicted read-demand / proximity-prior" estimator (A3/#4650/#4688) is **demoted to telemetry** (still trained and dashboarded, no longer part of the eviction key).
  - **There is no separate admission decision, no OOM valve, and no displacement-chain cap.** The **same** ranking runs whether the peer is relieving resource pressure or fitting a new arrival. A newcomer competes in this eviction like anything else; because it arrives with at most one subscriber it can never displace a 2+-subscriber incumbent, so multi-subscriber contracts are protected from churn for free, with no admission machinery; a fewest-subscriber newcomer is simply the one evicted (an emergent refusal). Subscribed contracts are **not** pinned or eviction-exempt: when the peer is over budget and holds nothing that ranks lower, a subscribed contract **is** evicted; a locally-subscribed one is evicted last but is still not absolutely pinned (no OOM-stuck corner). *Build note:* the shipped 0.2.90 base still pins subscribed contracts, still enforces a `min_ttl` floor, and still gates renewal on subscriptions only. Making reads/PUTs a retention-and-renewal signal, dropping `min_ttl`, and actually shedding a subscribed contract are the remaining steps; the last is release-2-scale (it needs subscription teardown, subscriber re-home via piece D, and splitting the combined subscriber count into the local/downstream dimensions above).
...
- **Pieces B and E fold into this model.** There is no separate **admission** decision (the old piece B): a full peer does not refuse a new subscription with a NACK. The newcomer competes in the single eviction of piece A, and a fewest-subscriber arrival is simply the contract evicted (emergent refusal). **Every-hop placement is already live**: a PUT stores at every hop and a GET stores at every return hop (pre-existing behavior, shipping in v0.2.93), governed by A's demand gauge. **E** removes only the **durable-regardless-of-demand** copy (the relay-caching anti-pattern), the redundant Source-2 fan-out, and GET-auto-subscribe (no durable subscription on an unasked GET); it does **not** remove the every-hop store, which stays gauge-governed and evictable. The durable-copy removal ([#4664](https://github.com/freenet/freenet-core/issues/4664)) is gated on A's gauge being **field-validated first**, because the durable every-hop copies are today's findability safety net: removing them before the gauge reliably keeps contracts placed could leave a contract with no copy anywhere.
...
- **SUBSCRIBE retirement + client deprecation adapter (R4, step 10).** The standalone SUBSCRIBE operation is retired in favor of a **`subscribe` flag on GET and PUT**: a new peer no longer emits a standalone SUBSCRIBE for an initial subscribe, because the subscribe rides on the GET (or PUT) that already routes toward the key, folding a wire round-trip and a duplicate code path into one. The standalone SUBSCRIBE **wire variant is not deleted**: it is **repurposed as `Renewal`** (the directed lease keep-alive of section 5, payload bincode-identical) and **kept in the enum**, because deleting a variant shifts bincode indices and is itself the cross-version break. Incoming standalone-SUBSCRIBE handling is retained through the rollout window so a new peer still accepts it from an old peer. The client-facing `ContractRequest::Subscribe` API **stays, deprecated**, translated by an adapter into GET-with-subscribe that synthesizes the `ContractResponse::SubscribeResponse` the client expects. This **is a version-gated wire change** (the subscribe flag on GET/PUT plus the repurposed variant), engineered to be non-breaking (see piece H); note the behavior change that a subscribe now pulls full state (a latency change on a live API), so smoke-test riverctl before release.
- **Piece H, mixed-version rollout.** Old and new nodes interoperate, but the redesign is **not** purely behavioral: R3/R4 add **version-gated wire changes**, namely the summary-first PUT message (the PUT protocol detail lives in the [build spec](hosting-spec.md)), a `subscribe` flag on GET and PUT, and the standalone SUBSCRIBE variant repurposed as `Renewal` (see the SUBSCRIBE-retirement piece above). These are engineered to be **non-breaking**: every new wire surface is gated on the fleet-minimum version, and enum variants are **kept and repurposed, never deleted**, so bincode indices do not shift. The `NetMessageV1::SubscribeHint` migration variant is likewise **kept** even though it is no longer emitted; do **not** remove it as part of this work (removing an enum variant shifts bincode indices and *is* the cross-version break). Retire kept-but-unemitted variants only as a separate later cleanup, once the whole fleet is past them.

---

## 10. The one-paragraph summary

Hosting is demand-driven: a peer hosts a contract because a local client, a farther-out subscriber, or a routed GET/PUT put real demand on it, held under a demand gauge and never durable-regardless-of-demand; there is no persistent "relay" role between hosting and not. State is an idempotent commutative monoid with no authoritative copy, and updates flow automatically between any two connected co-hosts, so a contract's update mesh is simply the connected sub-graph of its hosts. A subscribe builds a connected path of real hosts from the client's peer toward the key so that peer is both able to serve reads and kept fresh by proximity. Each peer maintains its place with one primitive, a renewal sent to its **computed** upstream (of its connected hosts that are strictly nearer the contract's key than itself, the most keyward one), gated by whether it still has demand, and both collapse-on-idle and re-rooting fall out of that one primitive. A peer keeps hosting only while it has a local client, a strictly-farther downstream, or recent real GET/PUT access, never because of a closer neighbor; that strict-distance rule makes the chain acyclic and its collapse terminating, which (with lease-expiring registrations) kills self-perpetuation, while interest-gated renewal kills the cache-proportional storm.
