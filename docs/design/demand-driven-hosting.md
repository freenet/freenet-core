...
- **Piece H, mixed-version rollout.** Old and new nodes interoperate, but the redesign is **not** purely behavioral: R3/R4 add **version-gated wire changes**, namely the summary-first PUT message (the PUT protocol detail lives in the build spec), a `subscribe` flag on GET and PUT, and the standalone SUBSCRIBE variant repurposed as `Renewal` (see the SUBSCRIBE-retirement piece above). These are engineered to be **non-breaking**: every new wire surface is gated on the fleet-minimum version, and enum variants are **kept and repurposed, never deleted**, so bincode indices do not shift. The `NetMessageV1::SubscribeHint` migration variant is likewise **kept** even though it is no longer emitted; do **not** remove it as part of this work (removing an enum variant shifts bincode indices and *is* the cross-version break). Retire kept-but-unemitted variants only as a separate later cleanup, once the whole fleet is past them.
