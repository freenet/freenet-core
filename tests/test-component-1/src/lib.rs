    ...
    fn process(messages: InboundComponentMsg) -> Result<Vec<OutboundComponentMsg>, ComponentError> {
        ...
        match messages {
            InboundComponentMsg::ApplicationMessage(incoming_app) => {
                let message: InboundAppMessage = serde_json::from_slice::<InboundAppMessage>(incoming_app.payload.as_slice())
                ...
                match message {
                    InboundAppMessage::CreateInboxRequest => {
                        ...
                        let secret_request = OutboundComponentMsg::SetSecretRequest(
                            SetSecretRequest {
                                key,
                        ...
                        let response_msg_content: OutboundAppMessage = OutboundAppMessage::CreateInboxResponse(public_key);
                    ...
                    InboundAppMessage::PleaseSignMessage(_) =>
                        return Err(ComponentError::Other(
                            format!("Unexpected app inbound message content")))
                }
            }
            _InboundComponentMsg => return Err(ComponentError::Other(
                format!("Unexpected app inbound message")))
        }
        Ok(outbound)
    }
}