    ...
    loop {
        tokio::select! {
            ...
            event = async {
                ...
                match event {
                    Some(_event_id) => {
                        // Sleep between events to allow network operations to process
                        // Using 75ms instead of original 200ms for faster test execution
                        // This provides ~2.7x speedup while allowing adequate processing time
