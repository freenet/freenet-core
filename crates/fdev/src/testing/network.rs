...
use freenet::dev_tool::{
    EventChain, InterProcessConnManager, MemoryEventsGen, NodeConfig, NodeLabel, OperationMode,
    PeerCliConfig, PeerId, SimPeer,
};
...
impl Display for Process {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Child(config) => write!(f, "child"),
        }
    }
}
...
pub(super) async fn run(
    ...
    match &cmd_config.mode {
        ...
        Process::Child(peer_cli_config) => {
            child(
                config,
                peer_cli_config,
                cmd_config.id.expect("id should be set for child"),
            )
            .await
        }
    }
...
async fn supervisor(config: &super::TestConfig) -> anyhow::Result<(), Error> {
    ...
    let cmd_args = SubProcess::build_command(config, seed);

    for (label, node) in &peers {
        let mut subprocess = SubProcess::start(
            &cmd_args,
            label,
            node.peer_id,
            node.local_ip.unwrap(),
            node.local_port.unwrap(),
        )?;
...
async fn child(
    ...
    peer_cli_config: &PeerCliConfig,
    id: usize,
) -> anyhow::Result<()> {
    print!("Inside child{} ", id);
    ...
    println!("Node config: {:?}", node_config.peer_id);
    ...
    let peer = node_config
        .build::<1>(
            peer_cli_config.clone(),
            [Box::new(event_generator)],
            private_key,
        )
        .await?;
...
mod tests {
    ...
    async fn test_child() {
        ...
        let network_config = NetworkProcessConfig {
            mode: Process::Child(peer_config),
            id: Some(0),
        };
