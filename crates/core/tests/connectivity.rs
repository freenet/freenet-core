...
/// Test three-node network connectivity to verify peer-to-peer mesh formation.
///
/// This test was previously removed because it revealed a bug where peers with only
/// gateway connections couldn't discover other peers (issue #1889). While PR #1890
/// fixed the topology manager to suggest diverse peer locations, the test still failed
/// because of a bug in the skip_list logic that prevented routing through the gateway.
///
/// This test verifies that:
/// 1. Peers can route through their gateway connection to discover other peers
/// 2. Connection maintenance successfully establishes a full mesh topology
/// 3. Contract operations work across the peer-to-peer mesh
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
