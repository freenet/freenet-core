...
#[derive(Clone)]
pub(super) struct HttpApiRequest(mpsc::Sender<ClientConnection>);

impl std::ops::Deref for HttpApiRequest {
...
/// Local HTTP API server for accessing and interacting with contracts.
pub struct HttpApi {
...
impl HttpApi {
    ...
    pub fn as_router_with_attested_contracts(
        ...
        let router = v1::routes(config.clone())
            ...
            .layer(Extension(HttpApiRequest(proxy_request_sender)));
...
async fn web_home(
    ...
    Extension(rs): Extension<HttpApiRequest>,
...
impl ClientEventsProxy for HttpApi {
    ...
    fn recv(&mut self) -> BoxFuture<'_, Result<OpenRequest<'static>, ClientError>> {
        async move {
            ...
            tracing::warn!("Shutting down HTTP API receiver");
