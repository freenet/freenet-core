...
async fn test_rampup_completes_with_high_queuing_delay() {
    ...
    while controller.congestion_state.load() == CongestionState::RampingUp && ramp_iterations < 30 {
        tokio::time::sleep(Duration::from_millis(55)).await;
...
#[tokio::test]
async fn test_variable_rtt_queue_buildup(#[case] base_rtt_ms: u64, #[case] queue_growth_ms: u64) {
    let min_cwnd = 2848usize;
    let config = LedbatConfig {
        initial_cwnd: 100_000,
        min_cwnd,
