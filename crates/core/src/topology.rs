...
impl TopologyManager {
    ...
    pub(crate) fn adjust_topology(
        ...
        if !suppress_swap && !matches!(adj, TopologyAdjustment::RemoveConnections(_)) {
            ...
            // Diversity shuffle: when topology swap returns NoChange, check whether
            // the neighborhood is both topologically suboptimal and stale. This
            // addresses neighborhoods that are stable but homogeneous — connections
            // that have been around for a long time without improvement.
            if matches!(adj, TopologyAdjustment::NoChange) {
                let shuffle = self.maybe_diversity_shuffle(
                    my_location,
                    neighbor_locations,
                    current_connections,
                    at_time,
    ...
    /// Diversity-driven peer-initiated shuffle.
    ///
    /// Fires independently from `maybe_swap_connection` when the neighborhood is
    /// both topologically suboptimal (gap excess) AND stale (many long-lived
    /// connections that haven't been replaced). Drops the oldest connection and
    /// targets a random location to inject fresh diversity.
    fn maybe_diversity_shuffle(
        &self,
        my_location: &Option<Location>,
        neighbor_locations: &BTreeMap<Location, Vec<Connection>>,
        current_connections: usize,
        ...
        // Reuse gap computation from maybe_swap_connection
        let signed_distances: Vec<f64> = neighbor_locations
            .keys()
            .map(|nloc| my_loc.signed_distance(*nloc))
            .collect();
        ...
        let gap_excess = ((largest_gap / expected_gap) - 1.0).clamp(0.0, 1.0);

        // Compute stale fraction: what proportion of connections are older than threshold
        let total_conns = neighbor_locations.values().map(|v| v.len()).sum::<usize>();
        if total_conns == 0 {
            return TopologyAdjustment::NoChange;
        }
        ...
        let deficit = gap_excess + STALENESS_WEIGHT * stale_fraction;

        if deficit < DIVERSITY_SHUFFLE_THRESHOLD {
            return TopologyAdjustment::NoChange;
        }

        // Shuffle probability proportional to deficit
        let shuffle_prob = ((deficit - DIVERSITY_SHUFFLE_THRESHOLD) / (1.0 + STALENESS_WEIGHT))
            .clamp(0.0, 1.0)
            * DIVERSITY_SHUFFLE_MAX_PROB;
        ...
        if roll >= shuffle_prob {
            trace!(
                ...
                shuffle_prob,
                roll,
                "Diversity shuffle check: not triggered"
            );
            return TopologyAdjustment::NoChange;
        }

        // Drop the oldest connection
        let oldest = neighbor_locations
            .values()
            .flatten()
            .min_by_key(|conn| conn.connected_at)
            .map(|conn| conn.location.clone());

        let Some(remove) = oldest else {
            return TopologyAdjustment::NoChange;
        };

        // Target the largest gap for replacement (same as swap)
        let add_location = small_world_rand::gap_target_directional(my_loc, &signed_distances);
        ...
        info!(
            ...
            remove_peer = %remove,
            add_target = %add_location,
            "Diversity shuffle triggered: replacing oldest connection with gap-targeted peer"
        );
