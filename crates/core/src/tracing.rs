...
pub struct ClientGetOutcomeSummary {
    ...
    /// Subset of `successes` that actually routed to the network
    /// (`attempts >= 1`), i.e. NOT a local-cache hit.
    pub network_successes: u64,
    /// Subset of `successes` served from a local copy with no network
    /// request (`attempts == 0`).
...
pub fn summarize_client_get_outcomes(logs: &[NetLogMessage]) -> ClientGetOutcomeSummary {
    ...
    // tx -> (outcome, attempts). Once-per-op by construction; the map dedups
    // any accidental double-registration to the last-seen terminal.
    let mut per_tx: HashMap<Transaction, (GetTerminalOutcome, usize)> = HashMap::new();
    for log in logs {
        ...
        }) = &log.kind
        {
            ...
            per_tx.insert(log.tx, (*outcome, *attempts));
        }
    }

    let mut summary = ClientGetOutcomeSummary::default();
    for (outcome, attempts) in per_tx.values() {
        match outcome {
