...
mod tests {
    ...
    fn chi_squared_test() {
        ...
        for i in 0..num_bins {
            ...
            expected_counts[i] = ((upper - lower) / (upper.powf(-1.0) - lower.powf(-1.0))).floor() * n as f64 / num_bins as f64;
        }
        
        let chi_squared = expected_counts.iter().zip(bins.iter()).map(|(&e, &o)| {
        ...
        assert!(p_value > 0.05, "Chi-squared test failed, p_value = {}", p_value);
    }
    ...
    fn metric_test() {
        ...
        assert!(metric.abs() < 0.1, "Small-world deviation metric is too high, metric = {}", metric);
    }
}
