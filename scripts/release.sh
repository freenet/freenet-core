...
Generated by: \`scripts/release.sh\`" \
    ...
    # Efficient polling - start fast, slow down over time
    local wait_time=5
    local max_wait=30
    local total_wait=0
    ...
    while true; do
        ...
        pr_state=$(gh pr view "$pr_number" --json state,mergeable,statusCheckRollup --jq '{state: .state, mergeable: .mergeable, checks: [.statusCheckRollup[]? | select(.status == "COMPLETED") | .conclusion]}')
        ...
        case "$state" in
            ...
            "CLOSED")
                ...
                ;;
            "OPEN")
                # Show progress on checks
                checks=$(echo "$pr_state" | jq -r '.checks | length')
                success_checks=$(echo "$pr_state" | jq -r '[.checks[] | select(. == "SUCCESS")] | length')
                if [[ $checks -gt 0 ]]; then
                    echo "  ⏳ CI progress: $success_checks/$checks checks passed"
                fi
                ;;
        esac
