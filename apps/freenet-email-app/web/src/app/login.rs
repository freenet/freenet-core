...
pub(super) fn LoggingScreen(cx: Scope) -> Element {
    ...
    use_shared_state_provider(cx, || CreateId(false));
    let create_id_form = use_shared_state::<CreateId>(cx).unwrap();
    ...
    let create_form_show = create_id_form.read().0;
    cx.render(rsx! {
        div {
            class: "columns",
...
struct CreateId(bool);

fn CreateLink(cx: Scope) -> Element {
    let _user = use_shared_state::<User>(cx).unwrap();
    let create_user_form = use_shared_state::<CreateId>(cx).unwrap();
    cx.render(rsx! {
        div {
            class: "box is-small",
            a {
