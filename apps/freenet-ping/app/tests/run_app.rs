...
use tokio::{select, time::timeout, time::sleep};
...
async fn test_ping_partially_connected_network() -> TestResult {
    ...
    // Configure logging
    freenet::config::set_logger(Some(LevelFilter::DEBUG), None);
    tracing::info!("Starting test with {} gateways and {} regular nodes (connectivity ratio: {})",
    ...
    for i in 0..NUM_GATEWAYS {
        ...
        let public_port = cfg.network_api.public_port.unwrap();
        let path = preset.temp_dir.path().to_path_buf();
        let config_info = gw_config(public_port, &path)?;
    ...
    for i in 0..NUM_REGULAR_NODES {
        ...
        tracing::info!("Node {} data dir: {:?} - Connected to {} other regular nodes (blocked: {})", 
                      i, 
    ...
    tracing::debug!("Test complete, dropping {} gateway presets and {} node presets", 
                   gateway_presets.len(), node_presets.len());
