...
async fn test_small_network_get_failure() -> TestResult {
    ...
    println!("   Simulating production conditions with {} total peers", NUM_GATEWAYS + NUM_NODES);
    
    // Create gateway
    let gateway = TestNodeBuilder::new(0)
        .gateway()
        .build()?;
    
    // Create nodes with limited connectivity
    // Node 1 connects only to gateway
    let node1 = TestNodeBuilder::new(1)
        .connect_to(&gateway)
        .build()?;
    
    // Node 2 connects only to gateway (no connection to node1)
    let node2 = TestNodeBuilder::new(2)
        .connect_to(&gateway)
        .build()?;
    ...
    //                              <- node2
    let node3 = TestNodeBuilder::new(3)
        .connect_to(&node1)
        .build()?;
