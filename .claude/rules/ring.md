...
Two prediction stacks exist in router.rs; exactly one reaches routing.
  ...
  HIERARCHICAL (default; router/hierarchical.rs): EB-shrunk isotonic prior,
    ...
    resolves OFF (=0, or any unrecognised or non-UTF-8 value) it is computed
...
WHEN touching either stack:
  ...
  → The resolved mode is logged ONCE, on first use (first route event,
    ...
    "disabled via" lines are INFO, so they are in the MAIN log (freenet.*,
    or the journal), NOT freenet.error.*, whose floor is WARN; only the
    unrecognised-value WARN line reaches the error log.
  → Routing-behaviour guards must cover BOTH stacks. Router::new(&history)
    ...
    timed events, so a guard whose property depends on timing must train
    that many and call assert_hierarchical_timing_decides (the realistic,
    transition phase-3 and #4230 steady-state twins do); otherwise its
    timing is legacy. Where both stacks agree bit for bit (all-success
  ...
  → With the default (on) it is always computed, because it routes. Only
    where the flag resolves OFF (=0, or any unrecognised or non-UTF-8
    value) is it computed solely while the
...
#5653 (failure labels) is deployed:
  ...
  (b) ON-FIELD CROSSOVER between gateways: one gateway on the hierarchical
      ...
      that node's MAIN log, freenet.* or the journal, not freenet.error.*
      (the line is INFO): "hierarchical routing estimator: disabled via
