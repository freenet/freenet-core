...
WHEN touching either stack:
  → Flags parse fail-safe. Default-off flags (parse_routing_flag, e.g.
    ...
    resolve_hierarchical_flag) is ON when unset/empty or 1/true/yes/on, OFF on
    0/false/no/off, and OFF with a warn! on ANY other non-empty or non-UTF-8
    value: setting a default-on flag at all almost always means "off", and
    ambiguous input falls back to the proven legacy stack.
  → The resolved mode is logged ONCE, on first use (first route event,
    ...
    them, and the exact strings are pinned end to end (real env var, real
    OnceLock, child processes) by
    hierarchical_routing_enabled_follows_the_environment.
  → Routing-behaviour guards must cover BOTH stacks. Router::new(&history)
    ...
    routes on the legacy fallback. The tests' Training::WarmHierarchical
    trains through add_event with a frozen clock and asserts the
    hierarchical estimate made the decision; give a new guard both modes.
  ...
  → With the default (on) it is always computed, because it routes. Only on
    a FREENET_ROUTING_HIERARCHICAL=0 node is it computed solely while the
...
#5653 (failure labels) is deployed:
  ...
  (b) ON-FIELD CROSSOVER between gateways: one gateway on the hierarchical
      ...
      that node's log ("hierarchical routing estimator: disabled via
