...
#
# Run: docker/freenet-gateway/test-entrypoint.sh

# shellcheck disable=SC2034
# Several variables below are consumed by freenet-gateway-startup.sh, which this
...
ENTRYPOINT="${HERE}/freenet-gateway-startup.sh"
...
# ---------------------------------------------------------------------------
# A stand-in for the gateway binary, written to $1 with version $2 baked in.
#
...
case "${1:-}" in
    ...
    network)
        ...
        # actually handed the gateway, not merely how it was invoked.
...
run_entrypoint() {
    (
        ...
        TRANSPORT_KEYPAIR="$WORK/keys/transport-keypair.pem" \
...
check "binary_version parses the gateway's --version output" \
...
# ---------------------------------------------------------------------------
# Binary seeding. The volume copy is what the gateway runs, and a self-updated
...
# Image moves ahead of the volume: pulling a newer image must upgrade the gateway.
...
check "the gateway was actually started for the clean-exit case" \
...
check "exit 43 starts the gateway exactly once" \
    1 "$(grep -c '^network ' "$STATE/network.log")"
...
check "the update ran with the exit status forwarded" \
    "update post_stop=42" "$(cat "$STATE/update.log")"
check "the gateway was started again after updating" \
    2 "$(grep -c '^network ' "$STATE/network.log")"
check "the update was applied to the binary the gateway runs from" \
...
# relied on the environment, so the gateway fell back to $HOME and wrote its
...
# and were simply empty. Assert on the ARGUMENTS the gateway receives, because that
# is the only place the mistake is visible.
# ---------------------------------------------------------------------------
fresh_run
printf '0\n' > "$STATE/exit-codes"
run_entrypoint > /dev/null
check "the gateway is given an explicit --config-dir" \
    1 "$(grep -c -- "--config-dir $WORK/config" "$STATE/network.log")"
check "the gateway is given an explicit --data-dir" \
...
check "the gateway is given --is-gateway" \
    1 "$(grep -c -- '--is-gateway' "$STATE/network.log")"
check "the gateway is given --skip-load-from-network" \
    1 "$(grep -c -- '--skip-load-from-network' "$STATE/network.log")"
check "the gateway is given --transport-keypair pointing at its own key" \
    1 "$(grep -c -- "--transport-keypair $WORK/keys/transport-keypair.pem" "$STATE/network.log")"
check "the gateway is given --public-network-address" \
    1 "$(grep -c -- '--public-network-address gateway.example.com' "$STATE/network.log")"

# ---------------------------------------------------------------------------
# A gateway's identity has to be stable: the same transport keypair across
# restarts and upgrades. Two properties matter, and each needs its own test --
# "a key exists after start" says nothing about whether a PRE-EXISTING key
# survived, and vice versa.
#
# The generated format is asserted directly (64 lowercase hex characters),
# because that is exactly what TransportKeypair::load (crates/core/src/
# transport/crypto.rs) requires: 32 raw bytes, hex-encoded. The previous
# version of this image generated an RSA PEM here, which the node has never
# accepted as a real key -- only detected as a legacy format and silently
# replaced on first use. A test that only checked "a file now exists" would
# have passed against that wrong format too.
# ---------------------------------------------------------------------------
fresh_run
rm -f "$WORK/keys/transport-keypair.pem"
printf '0\n' > "$STATE/exit-codes"
run_entrypoint > /dev/null
check "a missing transport keypair is generated on first start" \
    "yes" "$([ -f "$WORK/keys/transport-keypair.pem" ] && echo yes || echo no)"
check "the generated keypair is a 64-character hex string (32-byte X25519 secret)" \
    "yes" "$(awk 'END{print (length($0)==64 && $0 ~ /^[0-9a-f]+$/) ? "yes" : "no"}' "$WORK/keys/transport-keypair.pem")"

fresh_run
mkdir -p "$WORK/keys"
printf 'deadbeefcafedeadbeefcafedeadbeefcafedeadbeefcafedeadbeefcafedead' > "$WORK/keys/transport-keypair.pem"
printf '0\n' > "$STATE/exit-codes"
run_entrypoint > /dev/null
check "an existing transport keypair is preserved, not regenerated" \
    "deadbeefcafedeadbeefcafedeadbeefcafedeadbeefcafedeadbeefcafedead" \
    "$(cat "$WORK/keys/transport-keypair.pem")"

# ---------------------------------------------------------------------------
# PUBLIC_NETWORK_ADDRESS has no default -- a gateway started without it must
# fail immediately and say why, rather than silently starting a gateway
# nobody can reach.
# ---------------------------------------------------------------------------
fresh_run
: > "$WORK/entrypoint.log"
...
check "starting without PUBLIC_NETWORK_ADDRESS fails" 1 "$(cat "$WORK/status")"
check "the failure explains what is missing" \
    1 "$(grep -c 'PUBLIC_NETWORK_ADDRESS is not set' "$WORK/entrypoint.log")"
check "no gateway process was started without PUBLIC_NETWORK_ADDRESS" \
    0 "$(grep -c '^network ' "$STATE/network.log")"
...
# /data/logs on the host, died before starting the gateway, and every end-to-end
...
# leaves the gateway unable to start AND unable to update, backing off forever.
# ---------------------------------------------------------------------------
fresh_run
mkdir -p "$WORK/volume"
...
check "a corrupt volume binary present at startup is replaced" \
    0 "$(run_entrypoint)"
check "the gateway ran after that self-heal" \
    1 "$(grep -c '^network ' "$STATE/network.log")"
...
# without recovery the gateway can neither start nor update, so it backs off
...
check "an update that installs a broken binary self-heals on the next attempt" \
    0 "$(run_entrypoint)"
check "the gateway started again after the broken update was replaced" \
    2 "$(grep -c '^network ' "$STATE/network.log")"

# ---------------------------------------------------------------------------
# The single property this whole image exists to preserve: unless the operator
# explicitly opts out, the gateway must NOT be started with --disable-auto-update.
#
...
fresh_run
printf '0\n' > "$STATE/exit-codes"
run_entrypoint --max-connections 42 > /dev/null
check "operator arguments are forwarded to the gateway" \
    1 "$(grep -c -- '--max-connections 42' "$STATE/network.log")"
...
check "the gateway still restarts after a crash when opted out" \
    2 "$(grep -c '^network ' "$STATE/network.log")"
check "the gateway is told auto-update is disabled" \
...
# Without the opt-in the gateway never emits 45 at all, so the handling above would
# be dead code and the test above would pass regardless. Assert the marker the
# node actually receives.
check "the gateway is opted in to the distinct fast-crash exit code" \
    2 "$(grep -c 'fastcrash=1' "$STATE/network.log")"
check "the gateway is told it is supervised" \
...
check "the pid file is cleared after the gateway exits" \
    "no" "$([ -f "$WORK/node.pid" ] && echo yes || echo no)"

# ---------------------------------------------------------------------------
# SIGTERM must reach the gateway. This shell is PID 1, so if it does not forward
# the signal the gateway never drains and is SIGKILLed when the grace period ends,
...
check "SIGTERM was forwarded to the gateway so it could drain" \
    1 "$(grep -c '^terminated$' "$STATE/network.log")"
check "the gateway was not restarted after a stop signal" \
    1 "$(grep -c '^network ' "$STATE/network.log")"
check "no update is attempted while shutting down" \
    "" "$(cat "$STATE/update.log")"

# ---------------------------------------------------------------------------
# The same stop, but the gateway exits NON-ZERO on its way down. Without the
# stopping guard the loop reads that as a crash: it runs `freenet update` and
# restarts the gateway, in the middle of a container shutdown that Docker is
...
check "a non-zero exit during shutdown does not trigger an update" \
    "" "$(cat "$STATE/update.log")"
check "a non-zero exit during shutdown does not restart the gateway" \
    1 "$(grep -c '^network ' "$STATE/network.log")"

# ---------------------------------------------------------------------------
# A stop signal arriving DURING the update step, rather than while the gateway is
# running. The guard covering this is a different one from the guard the tests
# above exercise: those deliver TERM while the gateway runs, so the first check
...
check "a stop during the update step does not start the gateway again" \
    1 "$(grep -c '^network ' "$STATE/network.log")"

# ---------------------------------------------------------------------------
# `wait` returns 128+signum the instant a trapped signal arrives, while the gateway
...
# reason the gateway is given a 45s grace period.
# ---------------------------------------------------------------------------
fresh_run
printf 'sleep-drain\n' > "$STATE/exit-codes"
FREENET_CONFIG_DIR="$WORK/config" \
...
check "the supervisor waits for the gateway to finish draining" \
    "yes" "$([ "$drain_took" -ge 2 ] && echo yes || echo no)"
check "the gateway completed its drain before the supervisor exited" \
    1 "$(grep -c '^drained$' "$STATE/network.log")"

# ---------------------------------------------------------------------------
# The restart backoff must actually grow across consecutive failures, and reset
# once the gateway exits for a routine reason. A condition stuck at either extreme
# is invisible to any test that only checks the gateway restarted.
# ---------------------------------------------------------------------------
fresh_run
printf '101\n101\n101\n0\n' > "$STATE/exit-codes"
: > "$WORK/entrypoint.log"
...
# or already-current all land in the same branch, and the gateway still has to be
# restarted.
# ---------------------------------------------------------------------------
fresh_run
printf '101\n0\n' > "$STATE/exit-codes"
...
check "a failed update attempt does not kill the supervisor" \
    0 "$(run_entrypoint)"
check "the gateway is restarted after a failed update attempt" \
    2 "$(grep -c '^network ' "$STATE/network.log")"
